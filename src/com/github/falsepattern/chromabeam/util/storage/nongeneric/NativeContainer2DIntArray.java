...
public class NativeContainer2DIntArray {
    int[][][][] quadrants = new int[4][0][0][0];
    ...
    public void set(int x, int y, int[] data) {
        ...
        if (quadrant.length <= y) {
            var oldQ = quadrant;
            quadrants[id] = quadrant = new int[y + 256][0][0];
            System.arraycopy(oldQ, 0, quadrant, 0, oldQ.length);
        }
        var row = quadrant[y];
        if (row.length <= x) {
            var oldRow = row;
            quadrant[y] = row = new int[x + 256][0];
